## Dengue Outbreaks in Colombia: Epidemiological Features and Spatiotemporal Trends ####

# Juan D. Umaña, Julian Otero, Samuel Torres, Mauricio Santos-Vega (2023)

# R script for data import and preparation, and package install
...
years_to_analyze <- c(2007, 2008, 2009, 2010, 2011, 2012, 
                      2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021)

events_to_analyze <- c("DENGUE", "DENGUE GRAVE", "MORTALIDAD POR DENGUE")

tags_to_analyze <- c("CONSECUTIVE", "COD_EVE", "FEC_NOT", "CON_FIN",
                     "EDAD", "UNI_MED", "SEXO", "OCUPACION", "PER_ETN",
                     ...
                     "COD_PAIS_O", "COD_DPTO_O", "COD_MUN_O", "COD_DPTO_R", "COD_MUN_R", "COD_DPTO_N", "COD_MUN_N")
...
# epidata includes some notifications from 2006 ad 2022 that do not account for all regions.
# Hence, to avoid incomplete and bias towards under-reporting data notifications in those years are excluded.

data_Dengue <-  subset(data_Dengue, ANO_NOT>= 2007 & ANO_NOT <= 2021)

write.csv(data_Dengue, "data/data_dengue.csv", row.names = F)
